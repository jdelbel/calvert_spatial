...
data %>% 
  ...
  ggplot(aes(x = temp_dm, y = Hapto)) +
  geom_point(aes(fill = as.factor(year), shape = location), 
             size = 10, color = "black", stroke = 2, alpha = 0.9) +
  scale_fill_brewer(palette = "Dark2") +
  scale_shape_manual(values = c(21, 22, 23 ,24)) +
  labs(x = "Temp (°C)",
       y = bquote("Hapt. (mg" ~ m^-3*")")) +
  theme_bw() +
...
ggsave(here("figures_rda", "hapt_temp.png"), 
...
data %>% 
  filter(!is.na(location)) %>% 
  mutate(year = lubridate::year(date)) %>% 
  ggplot(aes(x = temp_dm, y = Cyanobacteria)) +
  geom_point(aes(fill = as.factor(year), shape = location), 
             size = 10, color = "black", stroke = 2, alpha = 0.9) +
  scale_fill_brewer(palette = "Dark2") +
  scale_shape_manual(values = c(21, 22, 23 ,24)) +
  labs(x = "Temp (°C)",
       y = bquote("Cyan. (mg" ~ m^-3*")")) +
  theme_bw() +
  theme(panel.background = element_blank(),
        panel.border = element_rect(fill = NA, colour = "black"), 
        legend.title = element_text(face = "bold", colour = "black"), 
...
ggsave(here("figures_rda", "cyan_temp.png"), 
       width = 10, height = 8, dpi = 300)
```
...
ggsave(here("figures_rda", "rda_2023-06-23_CHEMTAX_hellinger.png"),
