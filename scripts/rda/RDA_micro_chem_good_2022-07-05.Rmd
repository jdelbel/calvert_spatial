...
library(BiodiversityR) #Used for plotting ordinations in ggplot
library(ggrepel) #Helps with text in ggplot ordination plots
library(patchwork) #Allows combination of ggplot figures.
```
...
#Removing Metazoa and protists that couldn't be identified
micro <- micro %>% 
...
```{r}
#Filtering out species that weren't in at least 10% of samples
# sample_num <- micro %>% 
...
```{r}
#Pivoting longer the microscopy data
...
```{r}
expl_join <- mm_no_chl %>% 
  select(date:site_id, temp_dm:chl)
...
```{r}
#Testing to see if RDA appropriate for the data - length of first DCA is 1.05 which is < 3.S.D. The dataset is homogenous and a linear model is appropriate
dca <- decorana(resp)
dca
...
expl <- expl %>% 
  rename(temp = temp_dm, sal = sal_dm, drho = delta_rho_dm,
...
ggsave(here("figures_rev2", "rda_calvert_scaling2_2_het_2_pelag.png"),
