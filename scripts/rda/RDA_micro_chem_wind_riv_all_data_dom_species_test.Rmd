...
micro <- micro %>% 
  ...
  group_by(date, month, site_id, scientificName2) %>% 
...
micro_piv <- micro %>%
  mutate(month = lubridate::month(date)) %>% 
  select(date, month, site_id, scientificName2, species_sum)
...
sel <- goeveg::ordiselect(resp, rda.signif, fitlim = 0.99)
sel
...
ggsave(here("figures_rda", "rda_scale-micro_comb_riv_wind2_all_data_test.png"), micro_comb,
