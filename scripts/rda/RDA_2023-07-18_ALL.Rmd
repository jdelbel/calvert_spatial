...
pn %>% 
  ...
  ggplot(aes(x = temp, y = sal, fill = scientificName,
             size = species_sum/10^5, shape = site_id)) +
  geom_point() +
...
ggsave(here("figures_rda", "psn_t_s.png"),
...
micro_join %>%
  ...
  ggplot(aes(x = `Skeletonema marinoi`, y = drho, fill = as.factor(month))) +
...
env.signif <- subset(expl, select = c(secchi, sio2, temp, gm_b1))
...
```{r}
#Select the statistically significant variables - May want to remove SiO2, see VIFS below!
env.signif_d <- subset(expl, select = c(drho, sio2, temp, gm_b1))
...
ggsave(here("figures_rda", "rda_2023-07-18_all_s3.png"),
...
ggsave(here("figures_rda", "combined.png"), fig,
