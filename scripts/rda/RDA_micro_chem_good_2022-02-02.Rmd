...
```{r}
#Combined figure
fig <- p_s1_micro + p_s1_chem2

ggsave(here("figures_rev2", "rda_micro_chem_combined.png"),
...
env_loc_pca <- as.data.frame(pca$var$coord)
...
ggplot() + 
  ...
  #                        
  #                           lwd = 1) + 
  # geom_segment(data = env_loc_pca, aes(x = 0, y = 0, xend = PCA1, yend = ), 
  #               arrow = arrow(angle = 22.5,length = unit(0.35,"cm"),
  #                             type = "closed"),
  #               linetype = 1, size = 0.9, colour = "blue") +
  # geom_segment(data = sp_loc_pca, aes(x = 0, y = 0, xend = RDA1, yend = RDA2),
  #                arrow = arrow(angle = 22.5,length = unit(0.10,"cm"),
  #                              type = "closed"),
  #              linetype = 1, size = 0.9, colour = "darkgray") +
  # geom_text_repel(data = sp_loc_pca, aes(RDA1, RDA2, 
  #                                          label = row.names(sp_loc_pca)),
  #                 # box.padding = unit(0.5, 'lines'),
  #                 # point.padding = NA,
  #                 # point.padding = unit(2, 'lines')) +
  #                 segment.color = 'grey',
  ...
  #                 size = 8,
  #                 color = "black") +
  # geom_text_repel(data = env_loc_pca , aes(RDA1, RDA2,
  #                                           label = row.names(env_loc_pca)),
                  # box.padding = unit(0.8, 'lines'),
                  # point.padding = NA,
                  # point.padding = unit(1, 'lines'),
                  # segment.color = 'blue',
                  ...
                  # max.iter = 3e3,
                  # size = 8) +
  labs(x = paste("RDA 1 (", format(100 *pca$cont[[1]][2,1],
                                   digits = 3), "%)", sep = ""),
       y = paste("RDA 2 (", format(100 *pca$cont[[1]][2,2], 
                                   digits = 2), "%)", sep = "")) +
