...
```{r}
micro_join <- micro_join %>% 
  select(date:up_b1, `Skeletonema marinoi`,  `Chaetoceros tenuissimus`, 
         `Pseudo-nitzschia seriata`, `Pseudo-nitzschia`, Hillea, Teleaulax,
         `Phaeocystis pouchetii`, `Parvicorbicula socialis`)
```


```{r}
#Separating into response variables (microscopy counts) and Explanatory variables (Environmental)
resp <- micro_join[, 17:ncol(micro_join)]
...
ggplot() + 
  ...
  geom_point(data = sites_loc, aes(x = axis1, y = axis2,
                                   fill = as.factor(cluster_pca),
                                   shape = location), 
             size = 9, color = "darkgray", stroke = 1.5, alpha = 0.8) +
...
#                                    fill = as.factor(year),
#                                    shape = location), 
#              size = 8, color = "black", stroke = 1.5, alpha = 0.8) +
...
ggsave(here("figures_rda", "rda_2023-04-17_s2_cluster_pca_sio2.png"), 
...
```{r}
#Separating into response variables (microscopy counts) and Explanatory variables (Environmental)
resp_chem <- chem_join[, 3:10]

#Explanatory variables
expl_chem <- chem_join[, 11:ncol(chem_join)]
...
# resp.hell_chem <- log1p(resp_chem)
```
...
```{r}
#Select the statistically significant variables - May want to remove SiO2, see VIFS below!
env.signif_chem <- subset(expl_chem, select = c())
