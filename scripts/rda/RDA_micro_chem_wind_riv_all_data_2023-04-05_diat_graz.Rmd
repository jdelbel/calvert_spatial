...
graz <- micro %>% 
    filter(genus == "Gonyaulax" | genus == "Gymnodinium" | genus == "Prorocentrum" |
           ...
           # !scientificName_accepted == "Hillea" &
           #  !scientificName_accepted == "Teleaulax")
```
...
resp <- micro_piv[, 4:ncol(micro_piv)]
...
tbRDA.all <- rda(resp.hell ~ . , data = expl, scale = F)
...
env.signif <- subset(expl, select = c(drho, no2, temp, Wan_b1))
...
ggsave(here("figures_rda", "rda_2023-04-17_s3_cluster_pca_sio2_flag.png"), 
