...
micro_join %>% 
  ...
  ggplot(aes(x = up_b1, y = drho, color = site_id)) +
  geom_point() +
...
env.signif <- subset(expl, select = c(tchla, sio2, temp, gm_b1))
...
ggplot() + 
  ...
  geom_segment(data = env_loc, aes(x = 0, y = 0, xend = RDA1*20,
                                   yend = RDA2*20),
  ...
  geom_text_repel(data = sp_bf, aes(RDA1, RDA2,
                  ...
                  max.iter = 3e3,
                  size = 8) +
  geom_text_repel(data = env_loc , aes(RDA1*5, RDA2*5, 
