...
#Trying to remove species that have very few occurrences - trying this later
# micro_1_test <- micro_1 %>% 
#   group_by(scientificName_accepted, count) %>% 
...
# Select groups/species that I have classed as autotrophic
monthly_qu39 <- micro_2 %>%
  filter(trophicStatus == "auto")

#Summing counts into genus - turn off if I don't want genus level data
monthly_qu39 <- monthly_qu39 %>%
  group_by()

#Perform monthly averaging on QU39 data to make comparable to Calvert data
monthly_qu39 <- monthly_qu39 %>%
...
#Combining Calvert and Qu39 data where all of the qu39 data is included.
...
#This filters out the mostly autotrophic species
# micro <- micro %>%
#   filter(trophicStatus == "auto" &
...
#Monthly average of genus data 
micro_2_gen <- micro_2_gen %>% 
  mutate(month = lubridate::month(date),
         year = lubridate::year(date)) %>%
...
micro_gen <- rbind(micro_1_gen, micro_2_gen)
...
#Selecting columns
micro_gen <- micro_gen %>% 
  select(date, site_id, genus, count)
...
legend("topleft", "stress = 0.191", bty = "n", cex = 1)

# orditorp(nmds_all, display = "sites", cex = 0.70, air = 0.01, col = "white")

ordihull(nmds_all, groups = grp, display = "sites", draw = "polygon")

# ordihull(nmds_all, groups = micro_piv$season, draw = "polygon", lty = 1, col = "grey90")

# plot(nmds.spp.fit, p.max = 0.01, col = "black", cex = 0.7)


##Try not grouping to genus by determining relative abundance.
```
...
png(here("figures", "Calvert_qu39_summer_test.png"), width = 2200, height = 1000) 
