...
cor_fig2 <- (c1 + c2 + c3) / (c4 + c5 + c6)

ggsave(here("figures_species_timeseries", "cor_fig2.png"), cor_fig2,
...
cryp <- micro %>% 
  ...
  group_by(date, site_id) %>% 
  summarise(cryp = sum(species_sum)) %>% 
  ungroup()

ga <- micro %>% 
  filter(group == "Chlorophyta-Prasinophyta") %>% 
  group_by(date, site_id) %>% 
  summarise(ga = sum(species_sum)) %>% 
  ungroup()
...
d1 <- cors_2 %>% 
  # filter(season == "autumn") %>%
  ggplot(aes(y = het_sum/10^3, x = diat/10^5)) +
  geom_point(aes(fill = as.factor(season), shape = location), 
             size = 7, color = "black", alpha = 0.9, stroke = 1) +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8) +
  scale_fill_manual(values = c("dodgerblue3", "darkolivegreen", "red2", "goldenrod1")) + 
  scale_shape_manual(values = c(21, 22, 23)) +
  ylim(0, 25) +
  labs(x = bquote("Diatoms (x"~10^5~L^-1*")"),
       y = bquote("Microzoop. (x"~10^3~L^-1*")")) +
  theme_bw() +
  theme(axis.title = element_text(face = "bold", colour = "black"),
        axis.text = element_text(colour = "black"),
...
c1 <- cors_2 %>% 
  # filter(season == "autumn") %>%
  ggplot(aes(y = het_sum/10^3, x = cryp/10^5)) +
  geom_point(aes(fill = as.factor(season), shape = location), 
             size = 7, color = "black", alpha = 0.9, stroke = 1) +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8) +
  scale_fill_manual(values = c("dodgerblue3", "darkolivegreen", "red2", "goldenrod1")) + 
  ...
  ylim(0, 25) +
  xlim(0, 11) +
  labs(x = bquote("Crypt. (x"~10^5~L^-1*")"),
       y = bquote("Microzoop. (x"~10^3~L^-1*")")) +
  theme_bw() +
  theme(axis.title = element_text(face = "bold", colour = "black"),
        axis.text = element_text(colour = "black"),
...
g1 <- cors_2 %>% 
  # filter(season == "autumn") %>%
  ggplot(aes(y = het_sum/10^3, x = ga/10^5)) +
  geom_point(aes(fill = as.factor(season), shape = location), 
             size = 7, color = "black", alpha = 0.9, stroke = 1) +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8) +
  scale_fill_manual(values = c("dodgerblue3", "darkolivegreen", "red2", "goldenrod1")) + 
  scale_shape_manual(values = c(21, 22, 23)) +
  ylim(0, 25) +
  labs(x = bquote("Green (x"~10^5~L^-1*")"),
       y = bquote("Microzoop. (x"~10^3~L^-1*")")) +
  theme_bw() +
  theme(axis.title = element_text(face = "bold", colour = "black"),
        axis.text = element_text(colour = "black"),
...
```{r}
cor_fig3 <- (d1 + c1 + g1) / (da + ca + ga)

ggsave(here("figures_species_timeseries", "cor_fig_phyto_group.png"), cor_fig3,
       width = 16, height = 10, dpi = 300)
```

```{r}
#Try correlating different grazers with JUST cryptophyte abundance.

test <- hetero_genus_wide %>% 
  left_join(oli) %>% 
  pivot_longer(cols = c(Gymnodinium:Oligotrichea), names_to = "genus",
               values_to = "sum_genus") %>% 
  left_join(cors_2) %>% 
  select(date:location, cryp)
...
hill <- micro %>% 
  ...
  select(date, site_id, hill = species_sum)

tel <- micro %>% 
  filter(genus == "Teleaulax") %>% 
  select(date, site_id, tel = species_sum)
...
```{r}
test %>% 
  filter(sum_genus < 20000) %>% 
  # filter(season == "autumn") %>% 
  ggplot(aes(x = sum_genus, y = cryp)) + 
  geom_point() +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8) +
  facet_wrap(~genus, scales = "free")

ggsave(here("figures_species_timeseries", "cor_hetero_genus_cryp_all.png"),
       width = 16, height = 10, dpi = 300)
```
...
test %>% 
  filter(sum_genus < 20000) %>% 
  filter(season == "autumn") %>%
  ggplot(aes(x = sum_genus, y = hill)) + 
  geom_point() +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8) +
  facet_wrap(~genus, scales = "free")

ggsave(here("figures_species_timeseries", "cor_hetero_genus_hillea_autumn.png"),
       width = 16, height = 10, dpi = 300)
```
