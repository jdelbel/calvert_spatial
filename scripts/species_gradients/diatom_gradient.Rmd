...
micro_diat_gt5_perc %>%
  ...
  filter(year == 2019 & location == "S") %>% 
  ggplot(aes(x = date, y = species_sum/100000, 
             fill = scientificName)) + 
  geom_area(stat = "identity", position = "stack", color = "black") + 
  facet_grid(location ~ year) +
  scale_fill_manual(values = getPalette(colourCount)) + 
  theme_bw() +
  ylab("Abund. Diatoms (10^5 cells L)") +
...
env %>% 
  filter(location == "S") %>% 
  ggplot(aes(x = as.factor(month_surv), y = t, fill = as.factor(year))) + 
...
fec %>%
  ggplot(aes(x = as.factor(month_surv), y = fecal)) + 
  geom_bar(stat = "identity", position = "stack", color = "black") + 
  facet_grid(location ~ year) +
  scale_fill_manual(values = getPalette(colourCount)) + 
  theme_bw() +
