...
```{r}
micro_pn_c <- micro_diat_gt5_perc %>% 
  filter(location == "C") 

colourCount_c = length(unique(micro_pn_c$scientificName))
getPalette_c = colorRampPalette(brewer.pal(9, "Set1"))
```
...
ggsave(here("figures_good", "channel_compositions_drivers.png"), test, 
       width = 16, height = 13, dpi = 300)
...
ggsave(here("figures_good", "compositions_all_test.png"),
       width = 16, height = 12, dpi = 300)
```
```{r}
...
micro_all_chl %>%   
  ...
  geom_bar(aes(x = month_surv, y = species_sum/100000, 
           fill = scientificName), stat = "identity", position = "stack",
           color = "black") + 
  geom_line(aes(x = month_surv, y = chl), size = 1.5) +
  geom_point(aes(x = month_surv, y = chl), size = 2.0) +
  scale_fill_manual(values = getPalette_c(colourCount_c)) + 
  facet_grid(location ~ year) +
  theme_bw() +
  ylab("Phyto. Abundance (10^5 cells L), Chl (ug/L)") +
  theme(text = element_text(size = 25),
        axis.title.x = element_blank(),
        # axis.text.y = element_blank(),
        axis.text = element_text(colour = "black"),
...
ggsave(here("figures_good", "compositions_all_chl.png"),
       width = 16, height = 12, dpi = 300)
```
...
micro_join %>%   
  ...
  ggplot() + 
  geom_bar(aes(x = month_surv, y = species_sum/10000, fill = scientificName),
...
test <- micro %>% 
  filter(scientificName == "Prorocentrum minimum")
...
cor_het <- rbind(cor_het, fec_merge)

data <- cor_het %>% 
  left_join(cor_exp)

data <- data %>% 
  filter(!(name == "Prorocentrum minimum" |
           name == "Dinophysis" |
           name == "Dinophysis rotundata"))
...
ggsave(here("figures_good", "correlations_test.png"),
...
ggsave(here("figures_good", "correlations_test2.png"),
       width = 12, height = 14, dpi = 300)
