...
all <- micro %>% 
  group_by(date, site_id) %>% 
  summarise(sum_abund = sum(species_sum)) %>% 
  ungroup() %>% 
  left_join(chl)
```
...
```{r}
all %>% 
  ggplot(aes(x = sum_abund, y = tchla)) +
...
```{r}
pig_join %>% 
  ggplot(aes(x = Cryptophyta, y = Cryptophytes)) +
  geom_smooth(method = "lm", color = "black") +
  geom_point(size = 5) +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8)

cryp <- pig_join %>% 
  select(date, site_id, Cryptophyta, Cryptophytes)
```

```{r}
pig_join_phyto %>% 
  ggplot(aes(x = sf_3, y = sf_gff)) +
  geom_smooth(method = "lm", color = "black") +
  geom_point(size = 5) +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8) +
  labs(x = "CHEM Diatoms",
...
micro_dino <- micro %>% 
  filter(group == "Dinoflagellata") %>% 
  filter(!genus == "Gymnodinium" & !genus == "Gyrodinium" & !genus == "Protoperidinium") %>% 
  group_by(date, site_id) %>% 
  summarise(dino_sum = sum(species_sum)) %>% 
  ungroup()
