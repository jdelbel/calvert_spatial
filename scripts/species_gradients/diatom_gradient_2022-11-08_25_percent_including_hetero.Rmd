...
micro_all_gt10_perc <- micro_all %>% 
  mutate(scientificName2 = ifelse(max_abund < 200000 & group == "Bacillariophyta",
...
```{r}
# micro %>% 
#   filter(scientificName == "Choanozoa") %>% 
#   ggplot(aes(x = date, y = species_sum, color = site_id)) +
#   geom_line()
# 
# test <- micro %>% 
#   filter(scientificName == "Parvicorbicula socialis")  
```
...
ggsave(here("figures_species_timeseries", "compositions_all_25_perc_TEST.png"),
