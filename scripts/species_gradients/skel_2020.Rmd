...
```{r}
riv_20 <- riv %>% 
  filter(date > "2020-01-01" & Watershed_group == "Q_tot")
```
...
ctd %>%
  ggplot() +
  geom_line(aes(x = date, y = dep_26_dm, color = site_id)) +
  # scale_x_date(limits = as.Date(c("2019-07-01", "2019-07-20"))) +
  # scale_x_date(limits = as.Date(c("2020-06-20", "2020-07-10"))) +
  # ylim(60, 100) +
  scale_y_reverse() +
...
riv_19 <- riv %>%
  ...
  filter(year == 2019 & month > 5 & month < 10) 

riv_19 %>% 
  ggplot() +
  geom_area(data = riv_19 %>% filter(!Watershed_group == "Q_tot"),
            aes(x = date, y = Q/10^3, fill = Watershed_group),
            stat = "identity", position = "stack", color = "black", 
            size = 0.5) +
...
riv_18 <- riv %>%
  mutate(year = lubridate::year(date),
         month = lubridate::month(date)) %>% 
  filter(year == 2018 & month > 5 & month < 10) 
...
riv_20 <- riv %>%
  mutate(year = lubridate::year(date),
         month = lubridate::month(date)) %>% 
  filter(year == 2020 & month > 5 & month < 10) %>% 
  left_join(ctd)

riv_20 %>% 
  ggplot() +
  geom_area(data = riv_20 %>% filter(!Watershed_group == "Q_tot"),
            aes(x = date, y = Q/10^3, fill = Watershed_group),
            stat = "identity", position = "stack", color = "black", 
            size = 0.5) +
  geom_line(data = riv_20 %>% filter(site_id == "KC10"),
            aes(x = date, y = delta_rho_dm)) +
  # geom_line(data = riv_18 %>% filter(Watershed_group == "Q_tot"),
  #           aes(x = date, y = Q/10^3), color = "black", size = 1.5) +
  # scale_fill_d3(labels = c('glacier', 'snow', 'rain')) +
  scale_fill_brewer(palette = "Blues",
...
ctd %>% 
  filter(site_id == "KC10") %>%
  ggplot(aes(x = delta_rho_dm, y = sal_dm)) +
  geom_point() 
...
test2 <- sf %>% 
  filter(date == "2018-04-28" & line_out_depth == 5)
...
test4 <- micro %>% 
  mutate(year = lubridate::year(date)) %>% 
  filter(site_id == "QCS01" | site_id == "KC10") %>%
  filter(year == 2020 & scientificName == "Skeletonema marinoi") %>% 
  summarise(median = median(species_sum))
...
micro_join %>%
  filter(site_id == "KC10" | site_id == "QCS01") %>% 
  ggplot(aes(x = date, y = `Rhizosolenia setigera`/10^5, fill = site_id)) +
  geom_col(color = "black", width = 1, size = 1) +
  scale_fill_manual(values = c("#EFC000FF","#868686FF"),
                    labels = c("C", "F")) +
  labs(y = bquote("Abund. ("~10^5 ~ cells ~ L^-1*")")) +
  labs(fill = "") +
  scale_x_date(limits = as.Date(c("2018-07-01", "2018-08-31"))) +
  annotate("text", x = as.Date("2020-06-24"), y = 5, label = "2020 - S.marinoi"
           , size = 10) +
  theme_bw() +
  theme(text = element_text(size = 25),
        axis.text = element_text(color = 'black'),
...
ctd %>%
  ggplot() +
  geom_point(aes(x = date, y = sal_dm, color = site_id)) +
  # labs(y = bquote("UI. ("~10^2 ~ m^-3*~s^-1*")")) +
  scale_x_date(limits = as.Date(c("2018-07-01", "2018-08-31"))) +
  # ylim(-2, 2) +
  theme_bw() +
