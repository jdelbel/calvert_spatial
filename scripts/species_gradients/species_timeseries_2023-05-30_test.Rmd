...
f1 <- test2 %>%
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2021-01-01"))) +
  ylim(0, 32) +
  labs(y = bquote("Abund. ("~10^5 ~ cells ~ L^-3*")")) +
  theme_bw() +
  theme(text = element_text(size = 25),
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
```{r}
#Pulling out dominant microzooplankton grazers
hetero <- micro %>% 
  filter(group == "Ciliophora" | genus == "Gymnodinium" | genus == "Gyrodinium"
         | genus == "Protoperidinium" | genus == "Ceratium")

#Summing dinoflagellates to genus
hetero_genus_sum <- hetero %>% 
  filter(!group == "Ciliophora") %>% 
  group_by(date, site_id, genus) %>% 
  summarise(sum = sum(species_sum)) %>% 
  ungroup()

#Leaving ciliates to species, but using same format heterotrophic dinoflagellates for merging
ciliate_species_sum <- hetero %>% 
  filter(group == "Ciliophora") %>% 
  group_by(date, site_id, scientificName) %>% 
  summarise(sum = sum(species_sum)) %>% 
  ungroup() %>% 
  rename(genus = scientificName)

hetero_comb <- rbind(hetero_sum, ciliate_sum)

hetero_sum <- hetero %>% 
  filter(!group == "Ciliophora") %>% 
  group_by(date, site_id) %>% 
  summarise(sum_het = sum(species_sum)) %>% 
  ungroup()

ciliate_sum <- hetero %>% 
  filter(group == "Ciliophora") %>% 
  group_by(date, site_id) %>% 
  summarise(sum_cil = sum(species_sum)) %>% 
  ungroup()

graz_tot <- hetero_comb %>% 
  group_by(date, site_id) %>% 
  summarise(sum_tot = sum(sum)) %>% 
  ungroup()

hetero_wide <- hetero_genus_sum %>% 
  select(date, site_id, genus, sum) %>% 
  pivot_wider(names_from = genus, values_from = sum,
              values_fill = 0)

ciliate_wide <- ciliate_species_sum %>% 
  select(date, site_id, genus, sum) %>% 
  pivot_wider(names_from = genus, values_from = sum,
              values_fill = 0)
...
graz <- hetero_wide %>% 
  left_join(ciliate_wide) %>% 
  left_join(hetero_sum) %>% 
  left_join(ciliate_sum) %>% 
  left_join(graz_tot) %>% 
  left_join(chem) %>% 
  left_join(sf_wide) %>% 
  left_join(chl_wide)
...
graz %>% 
  ...
  ggplot(aes(x = sum_cil, y = tchla)) +
  geom_point() +
  xlim(0, 25000) 
  # ggpubr::stat_cor(p.accuracy = 0.001, rr.accuracy = 0.01, size = 8)

```


```{r}
hetero_wide <- hetero_sum %>% 
  select(date, site_id, genus, sum) %>% 
  pivot_wider(names_from = genus, values_from = sum,
              values_fill = 0) 

hetero_long <- hetero_wide %>% 
  pivot_longer(cols = c(Gymnodinium:Protoperidinium), names_to = "scientificName",
               values_to = "species_sum")

ciliate_wide <- ciliate_sum %>% 
  select(date, site_id, genus, sum) %>% 
  pivot_wider(names_from = genus, values_from = sum,
              values_fill = 0) 

ciliate_long <- ciliate_wide %>% 
  pivot_longer(cols = c(`Myrionecta rubra`:Tintinnina), names_to = "scientificName",
               values_to = "species_sum")


hetero_long2 <- hetero_long %>% 
...
ciliate_long %>%
  ggplot() + 
  geom_area(aes(x = date, y = species_sum, 
