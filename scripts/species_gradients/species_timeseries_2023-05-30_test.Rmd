...
sf_less_perc %>% 
  ...
  ggplot(aes(x = chla, y = perc, shape = site_id, color = as.factor(month))) +
  geom_point() +
  ylim(0, 1) +
...
sf_perc_wide <- sf_less_perc %>% 
  select(date:filter_type, perc) %>% 
  pivot_wider(names_from = filter_type, values_from = perc) %>% 
...
cor <- round(cor(cors_na, method = "pearson"), 2)

#Making significance' matrix
p.mat <- cor_pmat(cors_na, method = "pearson")
...
ggsave(here("figures_correlogram", "species_size_chem_pearson_SF.png"),
...
cors_diat %>% 
  filter(!is.na(scientificName)) %>% 
  ggplot(aes(x = Bacillariophyta, y = `20um`)) +
  geom_smooth(method = "lm", color = "black") +
  geom_point(aes(fill = scientificName), pch = 21, size = 5) +
  ggpubr::stat_cor(p.accuracy = 0.001, r.accuracy = 0.01, size = 8) +
  labs(x = "CHEM Diatoms",
...
#I want to look at the dominant species.
cors_dino <- pig_join %>% 
  select(date, site_id, dino, Dinoflagellata) %>% 
  left_join(main_dino)
...
cors_dino %>% 
  ...
  ggplot(aes(x = dino, y = Dinoflagellata)) +
  geom_point(aes(fill = scientificName), pch = 21, size = 5) +
...
#I want to look at the dominant species.
cors_dict <- pig_join %>% 
  select(date, site_id, dict, Dictyochophyta) %>% 
  left_join(main_dict)
...
cors_dict %>% 
  ...
  # filter(!scientificName == "Katodinium") %>% 
  # filter(!scientificName == "Dinophyceae") %>% 
  ggplot(aes(x = dict, y = Dictyochophyta)) +
  geom_point(aes(fill = scientificName), pch = 21, size = 5) +
  ggpubr::stat_cor(aes(label = ..rr.label..), size = 8) 
```
```{r}
#I want to look at the dominant species.
cors_cryp <- pig_join %>% 
  select(date, site_id, cryp, Cryptophyta) %>% 
  left_join(main_cryp)
...
cors_cryp %>% 
  ...
  # filter(!scientificName == "Katodinium") %>% 
  # filter(!scientificName == "Dinophyceae") %>% 
  ggplot(aes(x = cryp, y = Cryptophyta)) +
  geom_point(aes(fill = scientificName), pch = 21, size = 5) +
  ggpubr::stat_cor(aes(label = ..rr.label..), size = 8) 
...
meso2 <- micro %>% 
  filter(scientificName == "Myrionecta rubra")
```
```{r}
#I want to look at the dominant species.
cors_ga <- pig_join %>% 
