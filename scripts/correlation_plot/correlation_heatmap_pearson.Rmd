...
str(head(slice(data_nest_a, 1)))

cor_fun_a <- function(df) cor.test(df$vals,
                                 df$tchla,
                                 method = "pearson") %>% tidy()

data_nest_a <- mutate(data_nest_a, model = map(data, cor_fun_a))

corr_pr_a <- select(data_nest_a, -data) %>% unnest()

corr_pr_a

corr_pr_a <- mutate(corr_pr_a, sig = ifelse(p.value < 0.05, "Sig.", "Non Sig."))
```
...
corr_pr_a <- arrange(mutate(corr_pr_a,
...
```{r}
ggplot()+
  geom_tile(data = corr_pr_a,
            aes(group, name, fill = estimate),
            size = 1,
            colour = "white")+
  geom_tile(data = filter(corr_pr_a, sig == "Sig."),
            aes(group, name),
            size = 1.5,
            colour = "black",
            fill = "transparent") +
  geom_text(data = corr_pr_a,
...
ggsave(here("figures_cor", "all_pearson_test.png"),
...
ggsave(here("figures_cor", "dfo2_pearson_test.png"),
       width = 12, height = 12, dpi = 300)
...
ggsave(here("figures_cor", "qcs01_pearson_test.png"),
       width = 12, height = 12, dpi = 300)
