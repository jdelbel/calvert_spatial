...
ggsave(here("figures_rev2", "hc_average_all_spec_4_6clust.png"), p_dend,
...
```{r}
#This is a dendrogram in the order of the clustering including all species. Way to busy, but good to inspect.
...
```{r}
#Plotting the same as above (dendrogram order), but trying with only the indicator species (p <0.01)
...
```{r}
#Plotting the same as above (dendrogram order), but trying with only the indicator species (p <0.01). Trying again with no labels
...
```{r}
#Second approach to try to lineup dendrogram with heatmap.
p_dend2 <- ggplot() +
 geom_segment(data = dendrogram_segments, 
              aes(x = x, y = y, xend = xend, yend = yend)) +
 geom_segment(data = dendrogram_ends, aes(x = x, y = y.x, xend = xend,
...
```{r}
#Testing if I can merge the above with the dendrogram.
fig <- p_dend2 / h1_1 + plot_layout(heights = c(1, 3))

fig2 <- fig_nmds_horiz / fig

ggsave(here("figures_rev2", "heat_test_2.png"), fig2,
