...
```{r}
#Upload data
#Upload chemtax data from master standardization sheet
...
transform_rel_hc <- transform_rel

rownames(transform_hc) <- labs_list

rownames(transform_rel_hc) <- labs_list
```
...
#Calculating Bray-Curtis Distances - relative data
dis_rel <- vegdist(transform_rel_hc, method = "bray")

#Applying average linkage hierarchical clustering
cluster.average <- hclust(dis, "average")

#Applying average linkage hierarchical clustering
cluster.average_rel <- hclust(dis_rel, "average")
...
ggsave(here("figures_good", "hc_chemtax_2022-08-08.png"), p_dend,
...
ggsave(here("figures_good", "bar_chemtax_2022-08-08.png"), 
...
nmds_chem_panel <- (nmds_month + nmds_year) / chem_clust_h 

ggsave(here("figures_good", "nmds_chemtax_cluster_year.png"), nmds_chem_panel, 
...
c_long %>% 
  filter(phyto_group == "hapt" &
           location == "S") %>% 
  ggplot(aes(x = date, y = TChla_mean, color = phyto_group)) +
  geom_line() +
  geom_point()
```
