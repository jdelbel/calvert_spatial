...
t_abund_diat <- sqrt(species_diat)
```
...
nmds2 <- ggplot(data = data.scores_abund_diat, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(data = data.scores_abund_diat, aes(fill = as.factor(year), shape = site),
             size = 9, stroke = 0.1, color = "black") + 
  # scale_fill_manual(values = c("#D7191C","#FDAE61","#ABDDA4","#2B83BA")) +
  scale_shape_manual(values = c(21, 22, 23)) + 
...
ggsave(here("figures_nmds_new", "2023-07-11_Year.png"), 
