...
```{r}
rownames(t_abund) <- labs_list

res_avg <- simprof(data = t_abund, 
               method.distance = "braycurtis",
               method.cluster = "ward")

res_cmp <- simprof(data = t_abund, 
               method.distance = "braycurtis",
               method.cluster = "ward")
```

```{r}
simprof.plot(res_avg)
simprof.plot(res_cmp)

res_avg$significantclusters
res_cmp$significantclusters
```

```{r}
clust_sp_avg <- as.data.frame(do.call(rbind, res_avg$significantclusters))
clust_sp_cmp <- as.data.frame(do.call(rbind, res_cmp$significantclusters))

clust_sp_avg <- clust_sp_avg %>% 
  mutate(rn = row_number()) %>% 
  pivot_longer(c(V1:V16), names_to = "cluster", values_to = "sample_name") %>% 
  select(-cluster) %>% 
  rename(cluster_avg = rn) %>% 
  group_by(cluster_avg) %>% 
  distinct() %>% 
  ungroup()

clust_sp_cmp <- clust_sp_cmp %>% 
  mutate(rn = row_number()) %>% 
  pivot_longer(c(V1:V17), names_to = "cluster", values_to = "sample_name") %>% 
  select(-cluster) %>% 
  rename(cluster_cmp = rn) %>% 
  group_by(cluster_cmp) %>% 
  distinct() %>% 
  ungroup()
```
...
micro_piv <- micro_piv %>% 
  ...
  left_join(clust_sp_avg) %>% 
  left_join(clust_sp_cmp) 

micro_piv <- micro_piv %>% 
  relocate(clust_abund, .after = cluster) %>% 
  relocate(cluster_avg, .after = clust_abund) %>% 
  relocate(cluster_cmp, .after = cluster_avg)

micro_piv <- micro_piv %>% 
  filter(!cluster_cmp == 3) %>% 
  filter(!cluster_cmp == 10)
...
clust_abund <- micro_piv$cluster_cmp
...
data.scores_abund$clust_abund = micro_piv$cluster_cmp
...
ggplot(data = data.scores_abund, aes(x = NMDS1, y = NMDS2)) + 
  ...
  # geom_mark_hull(aes(color = season),
...
t_abund_2 <- t_abund %>%
  filter(!row.names(t_abund) %in% c("F2019-08-05","F2019-05-14"))
clust_abund2 <- micro_piv$cluster_cmp

iv <- labdsv::indval(t_abund_2, clust_abund2)
...
micro %>% 
  ...
  ungroup() %>% 
  left_join(clust_abund_join) %>% 
  ggplot(aes(x = as.factor(clust_abund), y = n, fill = as.factor(clust_abund))) +
  geom_boxplot() + 
  scale_fill_npg() +
  ggpubr::stat_compare_means(size = 6) +
  theme_bw() +
...
sf <- sf_join %>%
  ...
  ggplot(aes(x = as.factor(clust_abund), y = perc_sf, fill = as.factor(clust_abund))) +
  geom_boxplot() + 
  ylim(0, 1) +
  scale_fill_npg() +
  # ggpubr::stat_compare_means(size = 6) +
...
kruskal.test(perc_sf ~ clust_abund, data = sf_join_20)

sf_join_20$clust_abund <- as.factor(sf_join_20$clust_abund)

#Dunn Test to look where differences are occuring.
FSA::dunnTest(sf_join_20$perc_sf ~ sf_join_20$clust_abund,
              method = "bonferroni")
...
ggsave(here("figures_nmds_new", "dendro_box_2023-09-13.png"), fig,
...
ggsave(here("figures_nmds_new", "box_2023-09-13.png"), fig,
       width = 16, height = 16, dpi = 300)
```

```{r}
test <- data_pca_join %>% 
  filter(clust_abund == 4) %>% 
  mutate(clust_abund_2 = case_when(clust_abund == 4 & (date == "2018-05-23" |
                                                         date == "2020-08-07" |
                                                         date == "2019-06-11" |
                                                         date == "2020-06-04" |
                                                         date == "2019-09-02" |
                                                         ...
                                                         date == "2020-07-05") ~
                                     "4a",
                                   clust_abund == 4 & (date == "2018-03-27" |
                                                         date == "2019-03-15" |
                                                         date == "2019-10-01" |
                                                         date == "2018-04-22" |
                                                         date == "2019-04-18") ~
                                     "4b",
                                   TRUE ~ as.character(clust_abund)))

test %>% 
  ggplot(aes(x = as.factor(clust_abund_2), y = gm_b1, fill = as.factor(clust_abund_2))) +
  geom_boxplot() + 
  # ggpubr::stat_compare_means(size = 6) +
  # labs(y = "Temp. (°C)") +
  scale_fill_npg() +
