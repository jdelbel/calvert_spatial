...
rownames(spp.scrs2_dino) <- c("Proto.",
                              "S.tro.",
                              "Kat.",
                              "Proro")
...
ggsave(here("figures_good", "nmds_relative_panel_2022-10-27_smar_sep.png"), fig,
...
ggsave(here("figures_good", "indicator_all_rel_2022-10-27_sm_sep.png"), all,
...
#Making a heatmap of the indicator species
all_month <- micro %>%   
  filter(scientificName %in% inv_month_list |
           scientificName == "Skeletonema marinoi small") %>% 
  ggplot(aes(x = as.factor(date), y = scientificName, fill = sqrt(perc_total))) +
  geom_tile(color = "black", size = 1) +
  facet_grid(group ~ month , scales = "free", space = "free") +
  scale_fill_distiller(palette = "Spectral", name = expression(sqrt("%"))) +
  # scale_y_discrete(labels = c("Ceratium lineatum" = "C.lin.", 
  #                             "Chaetoceros debilis" = "C.deb.",
  #                             "Chaetoceros decipiens" = "C.dec.",
...
ggsave(here("figures_good", "indicator_all_month.png"), all_month,
        width = 16, height = 8, dpi = 300)
...
```
Let's try relative abundance on dinflagellates and add significant species to heatmap.
...
inv_site_list_dino <- inv_year_sp_dino$rn
```
...
month <- micro_diat %>%
  ggplot(aes(x = as.factor(date), y = 1, fill = as.factor(month_surv))) +
