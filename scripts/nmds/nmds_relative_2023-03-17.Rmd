...
t_abund <- log1p(species)
# t_abund <- decostand(species, "hell")
...
ac <- function(x){agnes(hc_rel, method = x)$ac}
map_dbl(m,ac)
```
...
fviz_dend(clust_ward_rel, cex = 0.5, k = 6, color_labels_by_k = TRUE)
```
...
ggplot() +
 ...
 scale_y_reverse(limits = c(2.5, -1)) +
 coord_flip() +
 theme_bw() +
...
ggsave(here("figures_nmds", "test-2023-03-17.png"),
...
ggsave(here("figures_nmds", "test_2023-02-15_rel_2.png"),
       width = 6, height = 12, dpi = 300)
```
...
# 
#All species
vf <- envfit(nmds_rel, t_rel, perm = 999)
# 
# #Diatoms
...
ggsave(here("figures_nmds", "nmds_2023-03-17.png"), fig,
...
f1 <- ggplot(data = data.scores_rel, aes(x = NMDS1, y = NMDS2)) + 
  geom_point(data = data.scores_rel, aes(fill = as.factor(year), shape = site),
             size = 7, stroke = 1.5) + 
  scale_fill_brewer(palette = "RdYlBu") +
  # scale_fill_manual(values = c("blue", "springgreen4", "black", "magenta")) + 
  scale_shape_manual(values = c(21, 22, 23 ,24)) + 
...
ggsave(here("figures_nmds", "nmds_2023-03-17_rel_2.png"), fig,
...
inv_clust_pca = multipatt(t_abund, clust_pca, duleg = T, func = "r.g",
...
#Indicators by station for diatoms
inv_clust_rel = multipatt(t_rel, clust_rel, func = "r.g", duleg = F,
                       control = how(nperm = 9999))
