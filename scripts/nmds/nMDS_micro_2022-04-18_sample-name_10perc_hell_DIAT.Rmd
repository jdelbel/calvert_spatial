...
micro <- micro %>% 
  filter(!scientificName_accepted == "Bacillariophyceae")
```
...
transform <- decostand(species, 'hell')
...
ggsave(here("figures_rev2", "hc_average_names_10perc_hellinger_DIAT.png"), p_dend,
...
```{r}
#Want to try ANOSIM on each year-location

micro_piv_2018 <- micro_piv %>% 
  filter(year == 2018) %>% 
  arrange(site_id, date)


species_2018 <- micro_piv_2018[, 8:99]


transform_2018 <- log10(species_2018 + 1)

micro_piv_2019 <- micro_piv %>% 
  filter(year == 2019) %>% 
  arrange(site_id, date)


species_2019 <- micro_piv_2019[, 8:99]


transform_2019 <- log10(species_2019 + 1)

micro_piv_2020 <- micro_piv %>% 
  filter(year == 2020) %>% 
  arrange(site_id, date)


species_2020 <- micro_piv_2020[, 8:99]


transform_2020 <- log10(species_2020 + 1)
```

```{r}
site_id_2018 <- micro_piv_2018$site_id
site_id_2019 <- micro_piv_2019$site_id
site_id_2020 <- micro_piv_2020$site_id
```
...
#Turning off for now because takes time to analyze.
ano_site_2018 = anosim(transform_2018, site_id_2018,
...
micro_piv_2018_fs <- micro_piv %>% 
  filter(year == 2018 & (location == "F" | location == "S")) %>% 
  arrange(site_id, date)


species_2018_fs <- micro_piv_2018_fs[, 8:99]


transform_2018_fs <- log10(species_2018_fs + 1)
...
```{r}
site_id_2018_fs <- micro_piv_2018_fs$site_id
site_id_2019 <- micro_piv_2019$site_id
site_id_2020 <- micro_piv_2020$site_id
```
...
ggsave(here("figures_good", "nmds_year_heat_hellinger_diat.png"), fig2,
...
ggsave(here("figures_good", "nmds_year_richness_hell_diat.png"), fig2,
...
ggsave(here("figures_good", "nmds_year_richness_2_hell_diat.png"), fig2,
...
ggsave(here("figures_good", "nmds_clust_richness_hell_diat.png"), fig2,
       width = 16, height = 15, dpi = 300)
