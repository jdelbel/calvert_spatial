...
```{r}
micro_piv %>% 
  ggplot(aes(x = as.factor(year), fill = as.factor(clust_abund))) +
...
```{r}
micro_all_25 <- micro_all %>%
  mutate(scientificName2 = ifelse(max < 0.30 & group == "Bacillariophyta",
...
```{r}
micro_all_25 <- micro_all_25 %>% 
  group_by(sample_name, scientificName2) %>% 
  summarise(species_sum = sum(species_sum)) %>% 
  ungroup()
```

```{r}
micro_all_c <- micro_all_25  
...
ggsave(here("figures_nmds", "AAA_clust_panel2.png"), cp,
...
ggsave(here("figures_nmds", "AAA_test2.png"), panel,
        width = 16, height = 8, dpi = 300)
```
