...
micro <- micro %>% 
 ...
 # micro <- micro %>% 
 #   filter(!scientificName_accepted == "Hillea" & 
 #            !scientificName_accepted == "Teleaulax")
...
```{r}
micro %>% 
  filter(scientificName == "Leptocylindrus danicus") %>% 
...
test <- micro %>%
  ...
  summarise(median_diat = median(species_sum)) %>% 
  ungroup() %>% 
  group_by(site_id) %>% 
  top_n(10)
...
rhizo <- micro %>%
  filter(scientificName == "Rhizosolenia setigera") %>% 
  group_by(site_id) %>% 
  summarise(num = n(),
            med = median(species_sum)) %>% 
  ungroup()
...
# t_abund <- tran(species, "rootroot")
t_abund <- decostand(species, "total")
# t_abund <- sqrt(t_abund)
# t_abund <- scale(species)
```
...
ggsave(here("figures_nmds_new", "sqrt_ra_diatoms.png"),
...
ggsave(here("figures_nmds_new", "NMDS_no_cryp_2023-04-28.png"),
...
micro_all_gt10_perc <- micro_all %>% 
  mutate(scientificName2 = ifelse(max < 0.25,
...
ggsave(here("figures_nmds_new", "dendro_test.png"), cp,
...
micro_all_c %>%  
  ...
  geom_bar(aes(y = sample_name, x = species_sum/100000, 
           fill = scientificName2), stat = "identity", position = "fill",
           color = "black", width = 1) +
...
ggsave(here("figures_nmds_new", "test_vert_diat_abund.png"),
...
ggsave(here("figures_nmds_new", "AAA_test2_60.png"), panel,
        width = 16, height = 8, dpi = 300)
```
...
f1 <- ggplot() +
  geom_col(data = micro_all_c3, 
           ...
           fill = scientificName2), position = "fill",
           color = "black") +
...
ggsave(here("figures_nmds_new", "test_diatom_RA.png"), fig,
