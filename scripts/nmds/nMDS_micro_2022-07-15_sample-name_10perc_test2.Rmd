...
ggsave(here("figures_rev2", "hc_average_names_10perc_test2.png"), p_dend,
...
p1 <- ggplot(data = data.scores, aes(x = NMDS1, y = NMDS2)) + 
  ...
  ggrepel::geom_text_repel(data = spp.scrs2, aes(x = NMDS1, y = NMDS2, 
                  ...
                  size = 6) +
  # xlim(-0.4, 0.7) +
  annotate("text", x = 0.60, y = 0.35, label = "a)", size = 10) +
  annotate("text", x = 0.40, y = -0.30, label = "Stress = 0.23", size = 10) +
...
p2 <- ggplot(data = data.scores, aes(x = NMDS1, y = NMDS2)) + 
  ...
  scale_shape_manual(values = c(21, 22, 23 ,24)) + 
  # xlim(-0.4, 0.7) +
  annotate("text", x = -0.70, y = 0.35, label = "b)", size = 10) +
  theme(axis.title = element_text(size = 25, face = "bold", colour = "black"),
        panel.background = element_blank(),
        panel.border = element_rect(fill = NA, colour = "black"), 
        axis.ticks = element_blank(),
...
ggsave(here("figures_good", "nmds_year_heat_test.png"), fig2,
