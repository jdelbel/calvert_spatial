...
wind_fig <- wind_mm %>% 
  ...
  geom_text(aes(angle= -mm_wd - 90), label= "⮕", size = 10) +
...
```{r}
ctd_dr <- ctd %>% 
  select(date, location, dr = delta_rho_dm) %>% 
   add_row(date = as.Date("2020-01-01"), location = "S",
           dr = NA) %>% 
   add_row(date = as.Date("2019-01-01"), location = "S",
           dr = NA)

ctd_dr <- arrange(mutate(ctd_dr, 
                      location = factor(location,
                                               levels = loc)))
```
...
```{r}
drf <- ctd_dr %>% 
  ggplot(aes(x = date, y = dr, color = location)) +
  geom_vline(xintercept = as.Date("2019-01-01")) +
  geom_vline(xintercept = as.Date("2020-01-01")) +
  annotate(geom = "rect", 
           xmin = as.Date("2018-03-01"), xmax = as.Date("2018-06-01"), 
...
```
```{r}
fig <- wind_fig / riv_fig / tf / drf / nf / chl_fig 

ggsave(here("figures_river", "monthly_means_dr.png"), fig,
       width = 16, height = 20, dpi = 300)
```
...
#Looking at seasonal statistics for the river, wind and upwelling data to see if there are any differences
wind <- env %>%
  select(date, wind_dm, wind_dir_dm) %>% 
  mutate(month = lubridate::month(date),
         year = lubridate::year(date)) %>% 
  mutate(season = case_when(month == 12 | month == 1 | month == 2 ~ "winter",
                            month >= 3 & month <= 5 ~ "spring",
...
wind_stat_season <- wind %>% 
  filter(!month == 12) %>% 
  group_by(season, year) %>% 
  summarise(mean_ws = mean(wind_dm),
            std_ws = sd(wind_dm),
            mean_wd = mean(wind_dir_dm),
            std_wd = sd(wind_dir_dm)) %>% 
  ungroup()
...
par_mm <- env %>%
  mutate(month = lubridate::month(date),
         year = lubridate::year(date)) %>% 
  group_by(month, year) %>% 
  summarise(mm_par = mean(par_dm, na.rm = T)) %>%
  ungroup() %>% 
  mutate(month_mid = 15) %>% 
  unite(date, c("year", "month", "month_mid"), sep = "-") %>% 
  mutate(date = lubridate::ymd(date))
...
par_mm %>% 
  ...
  scale_x_date(breaks = seq(as.Date("2018-01-01"), as.Date("2021-01-01"),
                            by = "4 months"), date_minor_breaks = "1 month",
               date_labels = "%b%y") +
  geom_col(fill = "#ffc245", color = "black") +
  labs(y = bquote("PAR")) +
  theme_bw() +
  theme(
        legend.position = "none",
...
ggsave(here("figures_river", "par_season_mm_x_label.png"),
       width = 16, height = 6, dpi = 300)

```

```{r}
#Creating PAR datasheet to calculate statistics from
par <- env %>% 
  select(date, par_dm)

#Separating the PAR data into my defined seasons
par <- par %>%
  mutate(month = lubridate::month(date),
         year = lubridate::year(date)) %>% 
  mutate(season = case_when(month == 12 | month == 1 | month == 2 ~ "winter",
...
#Removing december to only focus on Jan-Feb
par_season_stat <- par %>% 
  filter(!month == 12) %>% 
  group_by(season, year) %>% 
  summarise(mean = mean(par_dm),
            median = median(par_dm),
            std = sd(par_dm)) %>% 
  ungroup()
...
env_date <- env %>% 
  select(date, wind_b1, wind_dir_b1, par_b1) %>% 
  filter(date == "2020-05-01")
...
test <- ctd %>% 
  select(date, site_id, temp_dm, sal_dm, delta_rho_dm) %>% 
  filter(date == "2020-04-30")
```
