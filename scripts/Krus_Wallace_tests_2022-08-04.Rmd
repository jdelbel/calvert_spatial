...
kruskal.test(fecal ~ location, data = fec)
...
```{r}
ggboxplot(fec, x = "location", y = "fecal",
          fill = "location", palette = "jco", size = 0.5)+ 
  # stat_compare_means(comparisons = fc_cs_fs) + 
  stat_compare_means(label.y = 1200, size = 6) +
  theme_bw() +
  labs(y = expression("Fecal (pellets/L)")) +
  theme(legend.position = "none",
        text = element_text(size = 25),
        axis.title.x = element_blank(),
        axis.line = element_line(colour = "black"),
...
ggsave(here("figures_good", "kruskal_dunn_fecal.png"), 
...
ggsave(here("figures_good", "kruskal_dunn_fecal_year.png"), 
       width = 6, height = 4, dpi = 300)
```
```{r}
year_19_20 <- list(c("2019", "2020"))


fec %>%
  mutate(year = lubridate::year(date)) %>% 
  ggplot(aes(x = as.factor(year), y = fecal)) +
  geom_boxplot(fill = "grey") +
  geom_jitter(aes(fill = as.factor(month_surv), shape = as.factor(location)),
              size = 5, stroke = 1.5) +
  stat_compare_means(comparisons = year_19_20,
                     label.y = c(1300),
                     size = 7) +
  stat_compare_means(label.y = 1500, size = 6) +
  scale_fill_brewer(palette = "RdYlBu") +
  # scale_fill_manual(values = c("blue", "springgreen4", "black", "magenta")) + 
  scale_shape_manual(values = c(21, 22, 23 ,24)) +
  ylab("Fecal(pellets/L") +
  theme_bw() +
...
fec %>%
  ggplot(aes(x = as.factor(location), y = fecal)) +
  geom_boxplot(fill = "grey") +
  geom_jitter(aes(fill = as.factor(month_surv), shape = as.factor(year)),
              size = 5, stroke = 1.5) +
  # stat_compare_means(comparisons = year_19_20,
  #                    label.y = c(1300),
  #                    size = 7) +
...
ggsave(here("figures_good", "kruskal_dunn_fecal_location.png"), 
       width = 8, height = 6, dpi = 300)
```
...
```{r}
f1 <- ggboxplot(env, x = "location", y = "n_p",
          fill = "location", palette = "jco", size = 0.5)+ 
  stat_compare_means(comparisons = fc_fs, size = 7) + 
  stat_compare_means(label.y = 18, size = 6) +
  theme_bw() +
  labs(y = expression("N:P")) +
  theme(legend.position = "none",
        text = element_text(size = 25),
        axis.title.x = element_blank(),
        axis.line = element_line(colour = "black"),
...
f2 <- ggboxplot(env, x = "location", y = "n_s",
          fill = "location", palette = "jco", size = 0.5)+ 
  stat_compare_means(comparisons = fc, size = 7, 
                     label.y = c(0.65)) + 
  stat_compare_means(label.y = 1, size = 6) +
  coord_cartesian(ylim = c(0, 1)) +
  theme_bw() +
  labs(y = expression("N:S")) +
  theme(legend.position = "none",
        text = element_text(size = 25),
        axis.title.x = element_blank(),
        axis.line = element_line(colour = "black"),
...
f3 <- ggboxplot(env, x = "location", y = "p_s",
          fill = "location", palette = "jco", size = 0.5)+ 
  stat_compare_means(comparisons = fs, size = 7, 
                     label.y = c(0.15, 0.16, 0.17)) + 
  stat_compare_means(label.y = 0.22, size = 6) +
  coord_cartesian(ylim = c(0, 0.22)) +
  theme_bw() +
  labs(y = expression("P:S")) +
  theme(legend.position = "none",
        text = element_text(size = 25),
        axis.title.x = element_blank(),
        axis.line = element_line(colour = "black"),
...
ggsave(here("figures_good", "kruskal_dunn_ratios.png"), fig,
