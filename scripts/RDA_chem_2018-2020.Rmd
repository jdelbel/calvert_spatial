...
# ?ordiR2step
ordiR2step(rda(resp_chem.hell ~ 1, data = expl_chem), 
           scope = formula(tbRDA_chem.all), direction = "forward",
...
sel.osR2 <- ordiR2step (tb_rda.vasc.0, scope = formula (tb_rda.vasc.all), 
                        R2scope = adjR2_chem.tbrda, direction = 'forward', 
                        permutations = 999)
...
env.signif <- subset(expl_chem, select = c(temp, po4))
