...
#Uploading chemtax
chem <- read_csv(here("outputs", "chem_photo_piv_2018-2020_2.csv")) 
...
resp <- chem_merge[, 6:13]

#Things to add TChla, WSDP, iso depth, FWC...
expl <- chem_merge[, 14:ncol(chem_merge)]
```
...
resp.hell <- decostand(resp_chem,'hell')
```
...
env.signif <- subset(expl, select = c(temp, po4))
...
ggsave(here("figures_new", "rda_chem_ggplot_Chl_bad-nuts.png"),
...
plot(rda.signif, scaling = 2, type = "none", xlab = c("RDA1 (38%)"),
               ylab = c("RDA2 (2%)"), xlim = c(-0.5, 0.5), ylim = c(-1.1, 1.1))
...
#So since distance doesn't matter, I can make arrows longer with scaling 2? 
arrows(0,0, scores(rda.signif, display = "species", choices = c(1), scaling = 2)*2, 
            scores(rda.signif, display = "species", choices = c(2), scaling = 2)*2,
            col = "grey", length = 0)

text(rda.signif, scaling = 2, display = "bp", col = "blue",cex = 0.8, font = 2)

text(scores(rda.signif, display = "species", choices = c(1), scaling = 2)*2.3,
     scores(rda.signif, display = "species", choices = c(2), scaling = 2)*2.3,
     labels = rownames(scores(rda.signif, display = "species", scaling = 2)),
     col = "grey", cex = 1, font = 2)
