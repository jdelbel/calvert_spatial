...
```{r}
#Plotting Chl Timeseries
f1 <- data %>% 
  filter(!site_id == "QU39") %>% 
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = bulk_chl, group = site_id, fill = site_id, 
             color = site_id)) +
  geom_line(size = 2) +
...
fig <- f1/f2/f3/f4/f5/f6/f7

ggsave(here("figures_rev2", "test.png"), fig,
