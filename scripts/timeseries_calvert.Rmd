...
```
```{r}
#Summing species for the KC10-QU39 Time-series
...
#Plotting a scatterplot of the two variables including linear regression line and statistics
chem_compare %>% 
...
f1 <- chl_merge %>% 
  ...
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = chl, group = site_id, fill = site_id, 
             color = site_id)) +
  geom_line(size = 2) +
...
f2 <- data %>% 
  ...
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = temp_dm, group = site_id, fill = site_id, 
             color = site_id)) +
  geom_line(size = 2) +
  geom_point(pch = 21, size = 3, color = "black", stroke = 2) +
...
f3 <- data %>% 
  ...
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = sal_dm, group = site_id, fill = site_id,
             color = site_id)) +
  geom_line(size = 2) +
  geom_point(pch = 21, size = 3, color = "black", stroke = 2) +
...
f4 <- data %>% 
  ...
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = delta_rho_dm, group = site_id, fill = site_id,
             color = site_id)) +
  geom_line(size = 2) +
  geom_point(pch = 21, size = 3, color = "black", stroke = 2) +
...
f8 <- data %>% 
  ...
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = secchi_depth, group = site_id, fill = site_id,
             color = site_id)) +
  geom_line(size = 2) +
  geom_point(pch = 21, size = 3, color = "black", stroke = 2) +
...
ggsave(here("figures_rev2", "test5_chl_fixed.png"), fig,
...
```{r}
#Making a plot of river flow and DFO2 salinity and chl.

f1 <- riv %>% 
  filter(month > 4 & month < 11) %>% 
  ggplot(aes(x = as.factor(month), y = dis, fill = as.factor(year))) +
  geom_boxplot(position = position_dodge(preserve = "single"),
               color = "black", lwd = 0.7) +
  # geom_dotplot(aes(fill = as.factor(year)), color = "black", trim = FALSE,
...
f2 <- riv %>% 
  filter(yday > 100 & yday < 300) %>% 
  ggplot(aes(x = yday, y = dis, fill = as.factor(year), 
             color = as.factor(year))) +
  geom_line(size = 3) +
  # geom_point(pch = 21, size = 1, color = "black", stroke = 2) +
  geom_hline(yintercept = 5, linetype = "dashed", color = "black") +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1500)) +
  scale_x_continuous(expand = c(0, 0), limits = c(100, 300)) +
...
f3 <- chl_merge %>% 
  ...
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = chl, fill = as.factor(year), 
             color = as.factor(year))) +
  geom_line(size = 2) +
  geom_point(pch = 21, size = 3, color = "black", stroke = 2) +
  geom_hline(yintercept = 5, linetype = "dashed", color = "black") +
...
ggsave(here("figures_rev2", "riv_test.png"), fig,
...
ggsave(here("figures_rev2", "timeseries_calvert.png"),
...
micro_sum %>% 
  ...
  mutate(year = lubridate::year(date),
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  ggplot(aes(x = yday, y = sum/1000, group = group, fill = group)) +
  geom_area(position = "stack", alpha = 0.8, size = 0.5, colour = "black") +
  geom_point(aes(x = yday, y = sum_all/1000), size = 2) +
  scale_fill_manual(values = color_palette_micro) +
  facet_grid(site_id ~ year) +
  theme_bw() +
  theme(legend.position = "top",
...
ggsave(here("figures_rev2", "timeseries_calvert_ml.png"),
       width = 17, height = 10, dpi = 300)
...
micro_sum_2 %>% 
  ...
  geom_point(aes(x = yday, y = sum_all/1000), size = 2) +
  scale_fill_manual(values = color_palette_micro, 
                    labels = c("Bacillariophyta", #Y
                                    "Chrysophyta", #Y
                                    "Dictyochophyta", #Y
                                    "Raphidiophyta", #Y
                                    "Dinoflagellata", #Y
                                    "Cryptophyta",#Y
                                    "Chloro.-Prasinophyta", #Y 
                                    "Euglenophyta", #Y
                                    "Prym-Haptophyta", #Y 
                                    "Cyanobacteria" #Y
                                     #Y
...
ggsave(here("figures_rev2", "timeseries_kc10_qu39_ml.png"),
       width = 17, height = 10, dpi = 300)

```
```{r}
micro_all <- micro_kc10

micro_all <- micro_all %>% 
  filter(site_id == "QU39" | site_id == "KC10")
...
colourCount = length(unique(micro_all_gt10_perc$scientificName))
getPalette = colorRampPalette(brewer.pal(9, "Set1"))
```

```{r}
micro_all_c <- micro_all_gt10_perc  

colourCount_c = length(unique(micro_all_c$scientificName))
...
```{r}
micro_all_c_2 <- micro_all_c %>%
    complete(date, site_id, scientificName, fill = list(species_sum = 0))
...
micro_all_c_2 %>%
  ...
  ggplot() + 
  geom_area(aes(x = yday, y = species_sum/100000, 
           fill = scientificName), stat = "identity", position = "stack",
           color = "black") + 
  scale_fill_manual(values = getPalette_c(colourCount_c)) + 
  facet_grid(site_id ~ year, scales = "free_y") +
  theme_bw() +
  ylab("Phyto. Abundance (10^5 cells L)") +
  theme(text = element_text(size = 25),
        axis.title.x = element_blank(),
...
ggsave(here("figures_good", "compositions_all_test_kc10_qu39.png"),
