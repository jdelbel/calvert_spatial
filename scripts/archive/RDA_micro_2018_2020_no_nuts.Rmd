...
```{r}
#Removing nutrient columns
micro_merge <- micro_merge[,1:132]


#Remove rows with NaNs - some data with missing CTD - investigate
...
#Both constrained axes less than unconstrained
...
```
```{r}
# png(here("figures_new", "rda_micro_no_nuts.png"),
#      width = 800, height = 500)

plot2 <- plot(rda.signif, scaling = 2, type = "none", xlab = c("RDA1 (16%)"),
               ylab = c("RDA2 (3%)"))
...
sites.long2 <- sites.long(plot2, env.data = env.signif)
meta <- micro_merge[,5]
sites.long2 <- cbind(meta, sites.long2)
head(sites.long2)
...
axis.long2 <- axis.long(rda.signif, choices = c(1, 2))
axis.long2
...
ggplot() + 
    ...
    geom_point(data = sites.long2, 
               aes(x = axis1, y = axis2, color = site_id), 
               size = 5) +
  ...
  geom_text_repel(data = yz , aes(RDA1,RDA2,label=row.names(yz)))+
         labs(x=paste("RDA 1 (", format(100 *ii$cont[[1]][2,1], digits=2), "%)", sep=""),
              y=paste("RDA 2 (", format(100 *ii$cont[[1]][2,2], digits=2), "%)", sep=""))+
