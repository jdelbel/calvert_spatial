...
resp <- micro_merge[, 6:112]

#Things to add TChla, WSDP, iso depth, FWC...
expl <- micro_merge[, 113:ncol(micro_merge)]
```
...
# spe.hel <- decostand(resp,'hell')

spe.log10 <- log10(resp + 1)

# Calculate distance matrix
bc <- vegdist(spe.log10, method = "bray", binary = FALSE) 
...
simpleRDA <- rda(spe.hel ~ sal + temp + dr + turb + n2_max + no2 + sio2 + po4,  
                 data = micro_merge)
...
```
```{r}
ggord(simpleRDA, micro_merge$site_id) + 
...
spe.rda.all <- rda(spe.hel ~ sal + temp + dr + turb + n2_max + no2 + sio2 + po4,  
                 data = micro_merge)

ordistep(rda(spe.hel ~ sal + temp + dr + turb + n2_max + no2 + sio2 + po4,  
                 data = micro_merge), 
         direction = "forward", pstep = 1000, R2scop = TRUE)
```
```{r}
# Test of RDA result
...
env.signif <- subset(expl, select = c(sal, temp))
