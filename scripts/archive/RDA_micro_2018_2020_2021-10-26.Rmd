...
resp <- mm_no_chl[, 14:ncol(mm_no_chl)]
...
env.signif <- subset(expl, select = c(temp_dm, po4_dm, secchi_depth, sal_dm))
...
row.names(sp_loc_short_15p) <- c("P.n.m", 
                                 "P.n.s", 
                                 "Th", 
                                 "Din", 
                                 "Hill",
...
ggplot() + 
  ...
  geom_point(data = sites_loc, aes(x = axis1, y = axis2, fill = as.factor(month),
                                   shape = site_id), 
...
row.names(sp_loc_short_10p) <- c("P.n.m", 
                                 "P.n.s", 
                                 "Th", 
                                 "Din", 
                                 "Hill",
...
ggplot() + 
  ...
  theme(axis.title = element_text(face = "bold", colour = "black"),
        ...
        legend.title = element_text(face = "bold", colour = "black"), 
        legend.text = element_text(colour = "black"),
        legend.position = c(0.08, 0.34),
        legend.margin = margin(-0.3, 0, 0, 0, unit = "cm"),
        text = element_text(size = 10)) +
  guides(fill = guide_legend(override.aes = list(shape = 21))) +
  labs(fill = "Station", shape = "Season")

ggsave(here("figures_new", "rda_new_no_chla.png"),
       width = 6.2, height = 5, dpi = 300)
```
