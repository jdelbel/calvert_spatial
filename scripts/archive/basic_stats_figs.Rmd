...
```{r}
#Exporting data with calculations to incorporate into statistical analysis.

write_csv(ctd, here("outputs", "ctd_calcs.csv"))

write_csv(ctd_dd, here("outputs", "ctd_dd.csv"))

write_csv(ctd, here("outputs", "ctd_calcs.csv"))

write_csv(nuts, here("outputs", "nuts.csv"))
...
```{r}
hapto <- chem_tidy %>% 
  filter(phyto_group == "hapto" & site_id == "QCS01")


p1 <- chem_tidy %>% 
  filter(phyto_group == "hapto") %>%
  ggplot(aes(x = date, y = TChla, color = site_id)) +
  geom_line() +
  geom_point()

p2 <- chem_tidy %>% 
  filter(phyto_group == "hapto") %>%
  ggplot(aes(x = date, y = rel_abun, color = site_id)) +
  geom_line() +
  geom_point()
...
transect <- chem_tidy %>% 
  filter(date > "2019-08-02" & date < "2019-08-08")
...
transect %>% 
  ggplot(aes(x = as.factor(site_id), y = TChla, fill = fct_rev(phyto_group))) + 
  geom_bar(position = "stack", stat = "identity") +
  scale_fill_manual(values = palette_chem) 
```
```{r}
transect_ctd <- ctd %>% 
  filter(date > "2019-08-02" & date < "2019-08-08" & pres < 50)
...
transect_ctd %>% 
  ggplot(aes(x = sal, y = pres, color = station)) +
  geom_point() +
  scale_y_reverse()       

transect_ctd %>% 
  ggplot(aes(x = `Turbidity (FTU)`, y = pres, color = station)) +
  geom_point() +
  scale_y_reverse()       

transect_nuts %>%
  filter(line_out_depth < 21) %>% 
  ggplot(aes(x = no2_no3_um, y = line_out_depth, color = site_id)) +
  geom_point() +
  scale_y_reverse()  
...
```
```{r}
#Separating N2 out with monthly boxplot and timeseries
test <- ctd %>% 
  group_by(cast_pk) %>% 
  filter(N2 == max(N2))
...
p3 <- ctd %>% 
  ...
  filter(N2 == max(N2)) %>% 
  ungroup() %>%  
  ggplot(aes(x = date, y = N2, fill = station)) +
  geom_line(aes(color = station), size = 1.5) +
  geom_point(size = 4, pch = 21, color = "black") +
  # coord_cartesian(ylim = c(0, 25)) +
  labs(y = "Profile Max. N2",
...
p4 <- ctd %>% 
  ...
  filter(N2 == max(N2)) %>% 
  ungroup() %>%  
  ggplot(aes(x = date, y = pres, fill = station)) +
  geom_line(aes(color = station), size = 1.5) +
  geom_point(size = 4, pch = 21, color = "black") +
  # coord_cartesian(ylim = c(0, 25)) +
  labs(y = "pressure",
...
ggsave(here("figures_new", "box_maxN2-depth_2.png"), 
       fig, width = 18, height = 12, dpi=300)
```
