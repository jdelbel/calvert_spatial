...
up_dm %>% 
  mutate(month = lubridate::month(date),
         year = lubridate::year(date)) %>%
  ggplot() +
  geom_boxplot(aes(x = as.factor(month), y = dm_up, fill = as.factor(year)))
```
...
up_dm %>% 
  ...
  ungroup() %>% 
  filter(year < 2021) %>% 
  ggplot(aes(x = as.factor(month), y = mm)) +
...
data_long %>% 
  mutate(yday = lubridate::yday(date),
         year = lubridate::year(date)) %>% 
  filter(site_id == "QCS01" & param == "temp") %>% 
  ggplot(aes(x = yday, y = val, color = as.factor(year))) + 
  geom_point() +
  geom_line() 
```
