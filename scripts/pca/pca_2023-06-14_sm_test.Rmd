...
# ra <- decostand(species, method = "total")

ra_sm <- ra %>% 
  select(`Skeletonema marinoi`)

ra_sm$sample_name <- micro_piv$sample_name

ra_sm <- ra_sm %>% 
  select(sample_name, rel_sm = `Skeletonema marinoi`)
```
...
p4 <- ggplot(data = sites_loc_pca_clust, aes(x = axis1, y = axis2)) +
  ...
  scale_x_continuous(sec.axis = dup_axis(labels = NULL, name = NULL)) +
  scale_y_continuous(sec.axis = dup_axis(labels = NULL, name = NULL)) + 
  geom_point(data = sites_loc_pca_clust, aes(fill = rel_sm,
                                   shape = location, stroke = 0.1), 
             size = 10, color = "darkgray", alpha = 0.8) +
  # geom_mark_hull(data = sites_loc_pca_clust, aes(color = cluster, label = cluster),
  #                concavity = 2.8, label.fontsize = 30) + 
...
ggsave(here("figures_pca", "PCA-2023-05-30_all_data.png"), fig,
