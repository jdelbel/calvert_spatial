...
data_pca <- data_join %>%
  select(date, month, year, site_id, location, temp = temp_dm, 
         sal = sal_dm, drho = delta_rho_dm, DN = no2_dm,
         secchi = secchi_depth, sm_b1, gm_b1, ra_b1, wind_b1, 
         wind_dir_b1, up_b1, tchla, par_b1)
...
```{r}
data_pca %>% 
  ggplot(aes(x = date, y = drho, color = site_id)) +
  geom_line() +
  geom_point()
```
...
ggsave(here("figures_pca", "PCA-2023-10-18.png"), fig,
...
```{r}
#I think it might be important to highlight that 1) secchi depth is related to stratifiation at all stations and 2) Tchla is only related to secchi depth at KC10 and QCS01 and 3) stratification is related to discharge at all stations. Fuck all of the other correlations.

#How can I show this efficiently???

#Think of some sort of panel.

#If I can make a panel showing this, then I could tie stratification to discharge,

data_join %>% 
  ggplot(aes(x = sm_dis + gm_dis + ra_dis, y = delta_rho_dm, color = site_id)) +
  geom_point() 
