...
ggsave(here("figures_rev2", "diat_test_family.png"),
       width = 14, height = 8, dpi = 300)
```
```{r}
micro %>% 
  filter(scientificName == "Pseudo-nitzschia seriata"  & year == "2020") %>% 
  ggplot(aes(x = date, y = species_sum, color = location)) +
  geom_line() +
  geom_point()

micro %>% 
  filter(scientificName == "Rhizosolenia setigera"  & year == "2019") %>% 
  ggplot(aes(x = date, y = species_sum, color = location)) +
  geom_line() +
  geom_point()
