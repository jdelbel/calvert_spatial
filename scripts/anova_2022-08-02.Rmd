...
```{r}
pc <- data %>% 
  select(loc = location, t = temp_dm, s = sal_dm, dr = delta_rho_dm,
         no2 = no2_dm, sio2 = sio2_dm, po4 = po4_dm)
```

```{r}
ggboxplot(pc, x = "loc", y = "dr")
```
```{r}
pc %>% 
  group_by(loc) %>%
...
```{r}
pc <- pc %>% 
  mutate(s_log = log1p(s))

# Build the linear model
model  <- lm(dr ~ loc, data = pc)
...
```{r}
#So heterogeneity of variance assumption not met???

pc %>% levene_test(dr ~ loc)
```
```{r}
res.aov <- pc %>% anova_test(dr ~ loc)
res.aov
```
```{r}
pwc <- pc %>% tukey_hsd(dr ~ loc)
pwc
```

```{r}
res.kruskal <- pc %>% kruskal_test(dr ~ loc)
res.kruskal
```
```{r}
pc %>% kruskal_effsize(dr ~ loc)
```
```{r}
# Pairwise comparisons
pwc <- pc %>% 
  dunn_test(dr ~ loc, p.adjust.method = "bonferroni") 
pwc
```
