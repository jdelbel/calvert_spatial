...
fwi <- ctd %>%
  ...
  group_by(cast_pk) %>%
  filter(sal < 32.07 & pres > 2) %>%
  ungroup() %>%
  mutate(fwc_1 = ((32.07 - sal)/32.07)) %>%
  group_by(cast_pk) %>%
  mutate(fwc = sum(fwc_1)) %>%
  ungroup() %>%
  distinct(cast_pk, fwc)
...
ctd_calvert %>% 
  ...
  mutate(year = lubridate::year(date)) %>% 
  ggplot(aes(x = date, y = sal, color = as.factor(year))) +
  geom_point() 
...
ctd_5 %>%
  mutate(year = lubridate::year(date),
         month = lubridate::month(date)) %>%
  ggplot(aes(x = date, y = dep_26, color = as.factor(year))) +
  geom_line() +
  geom_point() +
  scale_y_reverse() +
  facet_grid(. ~ site_id)
...
fec %>% 
  filter(!is.na(site_id) & !site_id == "QU39") %>% 
  mutate(year = lubridate::year(date),
         yday = lubridate::yday(date)) %>% 
  filter(year > 2017 & year < 2021) %>% 
  ggplot(aes(x = yday, y = fecal, color = as.factor(year))) +
  geom_line() +
  geom_point() +
  facet_grid(site_id ~.)
...
write_csv(ctd_merge, here("outputs", "ctd_all_2023-03-30_fwi3.csv"))
# 
# #Exporting as a csv to pull into other analysis.
