...
ctd_5 <- ctd_5 %>% 
  group_by(date, site_id) %>% 
  summarise_at(vars(temp:dep_26), mean, na.rm = TRUE) %>% 
  ungroup()
...
ctd_merge_ds <- ctd_merge_no_qu39 %>%  
...
write_csv(ctd_merge_ds, here("outputs", "ctd_merge_2022-11-16_upper5_avg2.csv"))
```


```{r}
#Exporting as a csv to pull into other analysis.
# write_csv(micro, here("outputs", "micro_all_2022-07-29.csv"))
