...
wan_mean <- wan_ds %>%
  mutate(wan_b1 = rollmeanr(lag(dis, 0), k = 5, fill = NA))
...
db_sm <- db_ds %>% 
  ...
  mutate(sm_b1 = rollmeanr(lag(Q, 0), k = 5, fill = NA)) %>%
...
db_gm <- db_ds %>% 
  ...
  mutate(gm_b1 = rollmeanr(lag(Q, 0), k = 5, fill = NA)) %>%  
  select(!Watershed_group) %>%
  rename(gm_dis = Q)
...
db_ra <- db_ds %>% 
  ...
  mutate(ra_b1 = rollmeanr(lag(Q, 0), k = 5, fill = NA)) %>% 
  select(!Watershed_group) %>%
  rename(ra_dis = Q)

db_tot <- db_ds %>% 
  filter(Watershed_group == "Q_Total") %>% 
  mutate(tot_b1 = rollmeanr(lag(Q, 0), k = 5, fill = NA)) %>% 
  select(!Watershed_group) %>%
  rename(tot_dis = Q)
```
...
wind_mean <- lk_wind2 %>% 
  ...
  ungroup() %>% 
  rename(date = date2) %>%
  mutate(wind_b1 = rollmeanr(lag(wind_dm, 0), k = 5, fill = NA),
         wind_dir_b1 = rollmeanr(lag(wind_dir_dm, 0), k = 5, fill = NA)) %>% 
...
par_mean <- par %>% 
  ...
  ungroup() %>% 
  rename(date = date2) %>%
  mutate(par_b1 = rollmeanr(lag(par_dm, 0), k = 5, fill = NA)) %>% 
...
up_mean <- up2 %>% 
  ...
  ungroup() %>% 
  rename(date = date2) %>%
  mutate(up_b1 = rollmeanr(lag(up_dm, 0), k = 5, fill = NA)) %>% 
...
```{r}
write_csv(data, here("outputs", "enviro_2023-10-03_up_5day.csv"))
```
