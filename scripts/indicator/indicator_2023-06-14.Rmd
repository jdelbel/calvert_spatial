...
clust_pca <- read_csv(here("outputs", "pca_cluster_2023-06-14_upwell_tchla.csv"))
...
ano_site_pca = anosim(t_abund, site, distance = "bray", permutations = 9999)
ano_site_pca
...
inv_clust_pca = multipatt(t_abund_pca, clust_pca, func = "r.g", duleg = F,
                       control = how(nperm = 999))

#Looking at results of analysis
summary(inv_clust_pca)

iv <- labdsv::indval(t_abund_pca, clust_pca)
...
```{r}
micro_piv %>% 
  ggplot(aes(x = date, y = `Prorocentrum micans`,
             color = site_id)) +
  geom_point(size = 5) +
  geom_line()
```
...
#Indicators by station for diatoms
inv_clust_diat = multipatt(t_abund_diat, clust_pca, func = "r.g", duleg = T,
                       control = how(nperm = 999))
...
inv_clust_sp <- data.table::as.data.table(inv_clust_pca$sign, keep.rownames = TRUE)

inv_clust_sp <- inv_clust_sp %>%
  filter(s.1 == 1 | s.2 == 1 | s.3 == 1 | s.4) %>%
  filter(p.value < 0.05) %>%
  # filter(!rn == "Skeletonema marinoi") %>%
  arrange(rn)

inv_clust_sp_list <- inv_clust_sp$rn
...
pca <- micro %>%   
  # filter(!(pca_clust == 1)) %>% 
  filter(scientificName %in% inv_clust_sp_list) %>%
  ...
  ggplot(aes(x = pca_clust, y = scientificName, fill = sqrt(species_sum))) +
  geom_tile(color = "black", size = 1, alpha = 0.7) +
  facet_grid(pca_clust ~ , scales = "free", space = "free") +
...
loc <- micro %>%
  ...
  facet_grid(. ~ pca_clust , scales = "free_x", space = "free") +
  theme(axis.title.x = element_blank(),
        strip.background = element_blank(),
        strip.text.y = element_blank(),
...
ggsave(here("figures_indicator", "2023-07_04_pca_indicator.png"), panel,
