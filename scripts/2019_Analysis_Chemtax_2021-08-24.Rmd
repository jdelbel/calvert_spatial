...
#removing QU39
chem <- chem %>% 
  filter(!site_id == "QU39" & date > "2019-05-01" & date < "2019-11-01")
...
with(chem, legend("bottomright", legend = levels(as.factor(site_id)), bty = "n",
...
```{r}
png(here("figures_new", "Calvert_nmds_chemtax_2019-ts_species.png"), 
...
points(nmds_all, display = "sites", col = "black",
       ...
       bg = c("blue", "springgreen4", "black") [as.factor(chem$site_id)],
       cex = 3) 

# displays symbol and colour legend
legend("bottomright", legend = c(levels(as.factor(chem$season)), 
                              levels(as.factor(chem$site_id))),
       pch = c(21, 22, 23, 16, 16, 16),
       col = c("black", "black", "black", "blue", "springgreen4", "black"),
       bg = c(NA, NA, NA, "blue", "springgreen4", "black"),
       bty = "n", cex = 1.5)

# # displays legend text of stress value
legend("bottomleft", "stress = 0.10", bty = "n", cex = 1.5)
