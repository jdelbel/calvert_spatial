...
nmds <- micro_piv[, 6:147]

transform <- micro_piv[, 6:147]  
...
```{r}
plot(example_NMDS, type = "n", scaling = scl)

with(micro_piv, points(example_NMDS, display = "sites", col = colvec[as.factor(site_id)],
                      scaling = scl, pch = 21, bg = colvec[as.factor(site_id)]))
...
head(with(micro_piv, colvec[as.factor(site_id)]))
