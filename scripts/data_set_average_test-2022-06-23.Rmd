...
shannon_long <- shannon %>% 
  pivot_longer(!location:month_surv, names_to = "type", values_to = "value")
```

```{r}
shannon_long <- arrange(mutate(shannon_long,
                         location = factor(location, levels = order_loc)))
```


```{r}
fc_cs_fs <- list( c("F", "C"), c("C", "S"), c("F", "S"))
all_year <- list( c("2018", "2019"), c("2019", "2020"), c("2018", "2020"))
y18_20 <- list(c("2018", "2020"))
y18_19_20 <- list(c("2018", "2020"), c("2019", "2020"))
...
shannon_long %>% 
  ...
  geom_jitter(aes(fill = as.factor(month_surv), shape = as.factor(location)),
              size = 5, stroke = 1.5) +
  stat_compare_means(comparisons = all_year,
                     label.y = c(4,5,6),
                     size = 7) + 
...
ggsave(here("figures_good", "shannon_krus_test_year.png"), 
...
f1 <- shannon_long %>% 
  filter(type == "all") %>% 
  mutate(year = lubridate::year(date)) %>% 
  ggplot(aes(x = as.factor(location), y = value)) +
  geom_boxplot(fill = "grey") +
...
f5 <- shannon_long %>% 
  ...
  geom_jitter(aes(fill = as.factor(month_surv), shape = as.factor(location)),
              size = 5, stroke = 1.5) +
  stat_compare_means(comparisons = y18_20,
                     label.y = 3.5,
                     size = 7) + 
  stat_compare_means(label.y = 4, size = 6) +
  scale_fill_brewer(palette = "RdYlBu") +
...
f6 <- shannon_long %>% 
  ...
  geom_jitter(aes(fill = as.factor(month_surv), shape = as.factor(location)),
              size = 5, stroke = 1.5) +
  stat_compare_means(comparisons = y18_19_20,
                     label.y = c(3, 3.5),
                     size = 7) + 
  stat_compare_means(label.y = 4, size = 6) +
  scale_fill_brewer(palette = "RdYlBu") +
...
ggsave(here("figures_good", "shannon_krus_test_year_location.png"), fig, 
...
ggsave(here("figures_good", "shannon_test.png"), test,
...
```{r}
shannon_s %>% 
  ggplot(aes(x = as.factor(month_surv), y = dino_s, color = as.factor(year))) + 
  geom_point() 
