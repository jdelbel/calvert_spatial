...
ws844_2 <- ws844 %>%
  mutate(date = lubridate::mdy_hm(measurementTime),
         ...
         month = lubridate::month(date),
         yday = lubridate::yday(date)) %>% 
  select(date = date2, year, month, yday, lev = PLS_Lvl_Avg, rain = `24hourRain`) %>% 
  filter(year > 2017 & year < 2022)
```


```{r}
ws844_dm_rain <- ws844_2 %>% 
  group_by(date) %>% 
  summarise(dm_rain = mean(rain)) %>% 
  ungroup()
...
data_pca_long <- data_pca %>% 
  pivot_longer(cols = c(temp:dm_rain), names_to = "name", values_to = "vals")
```
...
f_ra <- riv_w_j %>%
  ggplot(aes(x = yday)) +
  geom_area(aes(y = dm_rain), size = 1, color = "purple", fill = "purple", alpha = 0.5) +
...
f3 <- riv_w_j %>% 
  ggplot(aes(x = yday)) +
  geom_area(aes(y = dm_par), size = 0, fill = "orange", alpha = 0.8) +
...
riv_w_j <- riv_w_j %>% 
  ...
  mutate(wind_vect2 = wind_vect/800) %>% 
  mutate(wind_u = wind_cube * cos(dm_wd)) %>% 
  mutate(wind_u2 = wind_u/800)



ylim.prim_wd <- c(-1.5, +1.5)   # in this example, precipitation
...
f5 <- riv_w_j %>% 
  ggplot(aes(x = yday)) +
  geom_line(aes(y = wind_vect2), size = 2.5, color = "black") +
  geom_line(aes(y = wind_u2), size = 1, color = "red") +
  # geom_smooth(aes(y = wind_vect2), span = 0.01, color = "purple", size = 2, se = F) +
...
```{r}
fig3 <- f_wannock / f_ra / f_d / f3 / f5 / f6 

ggsave(here("figures_timeseries", "DFO2_multiple_aligned_lookout_844rain_ethel_2023-01-19_WV.png"), fig3,
...
t1 <- riv_w_j %>% 
  ggplot(aes(x = yday)) +
  geom_point(aes(y = wind_vect2, fill = as.factor(year), color = as.factor(year))) +
  geom_line(aes(y = wind_vect2, color = as.factor(year))) +
  # geom_area(aes(y = wind_vect2), size = 0, fill = "purple", alpha = 0.3) +
  # geom_smooth(aes(y = wind_vect2, color = as.factor(year)),
  #             span = 0.05, size = 2, se = F) +
  # facet_grid(. ~ year) +
  theme_bw() +
  theme(text = element_text(size = 28),
        axis.text = element_text(color = 'black'),
...
ggsave(here("figures_timeseries", "DFO2_vect_test.png"),
        width = 16, height = 5, dpi = 300)
```
```{r}
t2 <- riv_w_j %>% 
  ggplot(aes(x = yday)) +
  geom_point(aes(y = Wan, fill = as.factor(year), color = as.factor(year))) +
  geom_line(aes(y = Wan, color = as.factor(year))) +
  # geom_area(aes(y = wind_vect2), size = 0, fill = "purple", alpha = 0.3) +
  # geom_smooth(aes(y = wind_vect2, color = as.factor(year)),
  #             span = 0.05, size = 2, se = F) +
  # facet_grid(. ~ year) +
...
ggsave(here("figures_timeseries", "DFO2_discharge_test.png"),
        width = 16, height = 5, dpi = 300)
```
```{r}
t3 <- riv_w_j %>% 
  filter(!is.na(tchla)) %>% 
  ggplot(aes(x = yday)) +
  geom_point(aes(y = tchla, fill = as.factor(year), color = as.factor(year))) +
  geom_line(aes(y = tchla, color = as.factor(year))) +
  # geom_area(aes(y = wind_vect2), size = 0, fill = "purple", alpha = 0.3) +
  # geom_smooth(aes(y = wind_vect2, color = as.factor(year)),
  #             span = 0.05, size = 2, se = F) +
  # facet_grid(. ~ year) +
...
ggsave(here("figures_timeseries", "DFO2_tchla_test.png"),
        width = 16, height = 5, dpi = 300)
```
...
ggsave(here("figures_timeseries", "test_panel.png"), tf,
