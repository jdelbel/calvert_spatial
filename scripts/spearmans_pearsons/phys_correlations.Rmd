...
riv2 <- riv %>%
  mutate(Riv = rollmeanr(lag(dis, 1), k = 4, fill = NA))

#25 ans 40 day back averages not doing anything
...
data_pca <- data %>% 
  select(date, month, month_surv, site_id, temp = temp_dm, sal = sal_dm, 
         ...
         secchi = secchi_depth, Riv, eth_w, eth_wd, lk_w,
         lk_wd, tchla)
...
```{r}
cor_exp <- rbind(s_sum, s_small, s_large, d_sum, c_sum, chl_corr, pn)
```

```{r}
data_pca_long <- data_pca %>% 
  pivot_longer(cols = c(temp:tchla), names_to = "name", values_to = "vals")
```
...
ggsave(here("figures_good", "correlations_test_pearson_S2.png"),
